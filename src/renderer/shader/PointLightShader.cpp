...
void PointLightShader::Vertex(glm::vec4& gl_Position, const VertexAttribute& vertex, unsigned gl_VertexIndex)
{
...
//return discard
bool PointLightShader::Fragment(glm::vec4& gl_FragColor)
{
	...
	glm::vec3 lightDir = glm::normalize(pLight.Position() - pos);
	float distance = glm::distance(pLight.Position(), pos);
	...
	glm::vec3 color = blinnPhongFactor * diffuseColor / (pLight.constant + pLight.linear * distance + pLight.quadratic * (distance * distance));
	glm::vec3 lightColor = pLight.Color();
